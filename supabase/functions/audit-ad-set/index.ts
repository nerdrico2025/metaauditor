...
serve(async (req) => {
    ...
    try {
        ...
        const { ad_set_id, date_start, date_end, force_refresh } = await req.json() as AuditRequest;
        if (!ad_set_id) throw new Error('Missing ad_set_id');

        const auditFocus = 'performance';

        if (!force_refresh) {
            const cached = await findRecentEntityAudit(supabase, {
            ...
            if (cached) {
                return new Response(JSON.stringify({ success: true, audit: cached, cached: true }), {
