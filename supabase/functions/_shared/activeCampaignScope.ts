...
export async function assertCreativeInActiveCampaign(
  ...
  let campaignStatus: string | null | undefined;
  const nested = creative.campaigns as { status?: string } | { status?: string }[] | null;
  if (Array.isArray(nested)) {
    campaignStatus = nested[0]?.status;
  } else {
    campaignStatus = nested?.status;
  }

  if (!campaignStatus && creative.campaign_id) {
  ...
  if (!creative.campaign_id || !campaignStatus) {
    return {
      ok: false,
