...
## ADDED (2026-06-05) — Colunas client-safe (Network / F12)

- Frontend **nunca** usa `select('*')` em `integrations`; apenas `INTEGRATION_PUBLIC_COLUMNS` (`src/lib/integrationColumns.ts`)
- Colunas **proibidas no browser**: `access_token`, `refresh_token`
- Migration `integrations_client` + `REVOKE SELECT` nas colunas sensíveis para `authenticated`/`anon`
- Edge Functions continuam com service role na tabela base
