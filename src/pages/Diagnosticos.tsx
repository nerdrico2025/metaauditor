...
import { dedupeLatestAudits, dedupeLatestEntityAudits, moduleToAuditFocus } from '@/lib/audit-focus';
...
import { fetchActiveCreativeIds } from '@/lib/fetchBrandingGateStatuses';
import {
    countByAuditStatus,
    matchesSearchFilter,
    matchesStatusFilter,
...
export default function Diagnosticos() {
    ...
    const hasActiveHistoryFilters =
        ...
        || ((historyScope as string) === 'campaign' && historyCampaignId !== 'all');
    ...
    const crossFocusCreativeIds = useMemo(
        () =>
            historyEntries
                .filter((entry) => entry.kind === 'creative' && entry.audit.creative_id)
                .map((entry) => entry.audit.creative_id as string),
        [historyEntries],
    );
    const { oppositeFocus: crossFocusOpposite, statusMap: crossFocusByCreativeId } =
        useCrossFocusStatusMap(crossFocusCreativeIds, module);
    ...
    const handleBatchAnalysis = async (ruleIds: string[], filteredCreativeIds?: string[]) => {
        ...
        try {
            ...
            const started = await runBatchAudit.mutateAsync({
                ...
                ...(filteredCreativeIds?.length ? { creativeIds: filteredCreativeIds } : {}),
        ...
        >
            ...
            <SelectRuleDialog
                ...
                onConfirm={(ids) => {
                    ...
                    void (async () => {
                        if (!companyId) return;
                        const creativeIds = await fetchActiveCreativeIds(supabase, companyId);
                        await runWithBrandingGate(creativeIds, (approvedIds) =>
                            handleBatchAnalysis(ids, approvedIds),
                        );
                    })();
                }}
