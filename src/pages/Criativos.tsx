...
export default function Criativos() {
    ...
    const { data: ruleChecksMap } = useCreativeRuleChecksBatch(isBranding ? creativeIds : []);
    ...
    const runAuditCreativeClientBatch = async (
        ...
        for (let i = 0; i < ids.length; i += BATCH_AUDIT_CONCURRENCY) {
            ...
            for (const pass of outcomes) {
                ...
                toastMod.toast.loading(`Auditoria IA: ${done}/${total} (audit-creative)…`, {
                    id: toastId,
                    duration: Infinity,
                });
            }
    ...
    const handleBatchAuditTopActive = async (ruleIds: string[]) => {
        ...
        try {
            progressId = toastMod.toast.loading('Buscando top 50 ativos…', { duration: Infinity });
            const ids = await fetchTopActiveCreativeIds();
            if (!ids.length) {
        ...
        >
            ...
            <motion.div variants={item} className="flex flex-wrap items-center justify-end gap-2 md:gap-3">
                ...
                >
                    ...
                    {activeOnly ? 'Mostrando apenas ativos' : 'Mostrar apenas criativos ativos'}
                </Button>
            </motion.div>
                        ...
                        {viewMode === 'grid' ? (
                            <div className={creativeGalleryGrid}>
                                {creatives.map((creative) => {
                                    ...
                                    const isWarning = isBranding && ruleCheck?.overall_status === 'warning';
                                    ...
                                    return (
                                        <motion.div key={creative.id} variants={item} className="min-w-0">
                                            <CreativeGridCard
                                                creative={creative}
                                                ...
                                                ruleCheckStatus={ruleCheck?.overall_status}
                                                brandingStatus={brandingCompliance?.byCreative.get(creative.id) ?? null}
                                ...
                                <div className="divide-y divide-border/50">
                                    {creatives.map((creative) => {
                                        ...
                                        const isRejectedList = isBranding
                                            ? listRuleCheck?.overall_status === 'rejected'
                                            : perfViolations.length > 0;
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`p-6 flex items-center gap-8 hover:bg-muted/50 transition-all group cursor-pointer ${
                                                    isRejectedList ? 'bg-rose-500/5 border-l-4 border-l-rose-500' : ''
                                                }`}
                                            >
                                                    ...
                                                    {creative.image_url ? (
                                                        isBranding ? (
                                                            <CreativeCompliancePreview
                                                                ...
                                                                name={creative.name}
                                                                status={listRuleCheck?.overall_status ?? 'pending'}
                                                ...
                                                {isBranding ? (
                                                    <div className="hidden lg:flex items-center gap-4 px-8 border-x border-border/50">
                                                        {(() => {
                                                ...
                                                <div className="hidden lg:flex items-center gap-12 px-12 border-x border-border/50">
                                                ...
                                                <div className="flex items-center gap-4 justify-end">
                                                    ...
                                                    <Button
                                                        ...
                                                        className={`rounded-xl border h-9 w-9 flex-shrink-0 transition-all ${listRuleCheck?.overall_status === 'approved' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400 hover:bg-emerald-500 hover:text-black' : listRuleCheck?.overall_status === 'rejected' ? 'bg-red-500/10 border-red-500/20 text-red-400 hover:bg-red-500 hover:text-white' : listRuleCheck?.overall_status === 'warning' ? 'bg-amber-500/10 border-amber-500/20 text-amber-400 hover:bg-amber-500 hover:text-black' : 'bg-muted border-border text-muted-foreground hover:bg-blue-500 hover:text-white hover:border-blue-500'}`}
