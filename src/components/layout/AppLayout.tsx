...
export function AppLayout({ requireAuth = true }: AppLayoutProps) {
    ...
    const supervisorPaths = ['/supervisor'];
    ...
    if (!isPlatformSupervisor && supervisorPaths.some(p => location.pathname.startsWith(p))) {
        toast.error('Acesso restrito ao painel de supervisão.');
        return <Navigate to="/dashboard" replace />;
    }
