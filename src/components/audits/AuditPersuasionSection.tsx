...
import { getScoreBgColor, getScoreColor, getScoreLabel } from '@/lib/audit-scores';
...
export function AuditPersuasionSection({
    ...
    const hasContent = found.length > 0 || missing.length > 0 || typeof ai?.persuasion_score === 'number';
    ...
    return (
        <div className={`space-y-3 ${compact ? '' : 'p-6 rounded-2xl border border-border bg-card'}`}>
            <div className="flex items-center justify-between gap-2">
                ...
                {ai && typeof ai.persuasion_score === 'number' && (
                    <Badge variant="outline" className={`${getScoreBgColor(persuasionScore)} ${getScoreColor(persuasionScore)} border-0 text-[10px]`}>
