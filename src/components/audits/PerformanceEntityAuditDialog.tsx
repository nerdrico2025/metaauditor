...
export function PerformanceEntityAuditDialog({
    ...
    const textAccent = score >= 70 ? 'text-emerald-500' : score >= 50 ? 'text-ch-orange' : 'text-rose-500';
                ...
                <div className="shrink-0 px-6 py-5 border-b border-border bg-gradient-to-br from-primary/10 to-background">
                    <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="text-center shrink-0">
                            ...
                            <p className={`text-3xl font-bold ${textAccent}`}>{score}%</p>
                        </div>
                    </div>
                </div>
                                ...
                                {(rollup.length ? rollup : perfRules).map((row) => (
                                    ...
                                    >
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            {'violating_creative_count' in row && (row.violating_creative_count ?? 0) > 0 && (
                                                <p className="text-xs text-rose-600/90 mt-1">
                                                    {row.violating_creative_count} criativo(s):{' '}
                                                    {(row.violating_creative_names ?? []).join(', ')}
                                                </p>
                                            )}
                                        </div>
                                    </li>
