...
import { AuditFrameworkGrid } from '@/components/audits/AuditFrameworkGrid';
...
import { auditFocusLabel, primaryAuditScore, resolveAuditFocus } from '@/lib/audit-focus';
import {
...
function buildOpportunitiesList(
    ...
    return (rawAnalysis.weaknesses ?? []).filter(w => {
...
function buildNeutralVerdict(
    ...
    const scoreVal = primaryAuditScore(audit, focus);
    const focusLabel = focus === 'branding' ? 'branding' : 'performance';
    if (problemsCount === 0) {
        return `Análise concluída com base nas regras e políticas configuradas. Score de ${focusLabel}: ${scoreVal}%. Nenhum problema detectado.`;
    }
    return `Análise concluída com base nas regras e políticas configuradas. Score de ${focusLabel}: ${scoreVal}%. ${problemsCount} problema(s) encontrado(s).`;
}
...
function resolveAuditDisplayData(
    ...
    const problems = buildProblemsList(audit.issues, perfResults, brandingResults);
    const strengths = buildStrengthsList(rawAnalysis, perfResults, brandingResults, isBranding);
    const opportunities = buildOpportunitiesList(rawAnalysis, problems);
    const suggestions = rawAnalysis.suggestions ?? rawAnalysis.action_plan ?? audit.recommendations ?? [];

    const tone_analysis =
        rawAnalysis.executive_summary ??
        rawAnalysis.tone_analysis ??
        (hasFrameworkScoresFlag ? '' : buildNeutralVerdict(audit, focus, problems.length));
...
export function CreativeAuditReportDialog({
    ...
    const rawAnalysis = audit?.ai_analysis ?? {};
    const focus: AuditFocus = auditFocusProp ?? resolveAuditFocus(audit);
    const isBranding = focus === 'branding';
    ...
    const resolved = useMemo(
        () =>
            audit
                ? resolveAuditDisplayData(rawAnalysis, audit, lastRulesCheck, lastPerfResults, focus)
                : null,
        [rawAnalysis, audit, lastRulesCheck, lastPerfResults, focus],
    );
    ...
    const improvementActions = useMemo(
        ...
        [creative, audit, resolved, focus],
    );

    if (!audit || !resolved) return null;
            ...
            {hasBranding && (
                <>
                    <div className="flex items-center justify-between">
                        ...
                        {lastRulesCheck?.overall_score !== undefined && (
                            <span className="text-xs font-bold">Score: {lastRulesCheck.overall_score}/100</span>
                        )}
                    </div>
