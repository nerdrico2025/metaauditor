...
import { Target, DollarSign, Video, Image as ImageIcon, Wand2, Loader2, Lightbulb } from "lucide-react";
...
export function FormatBattleChart({ video, image, topVideo, topImage }: FormatBattleChartProps) {
    ...
    const wins = {
        ...
        ctr: video.ctr > image.ctr ? 'video' : image.ctr > video.ctr ? 'image' : 'draw',
    };
            ...
            <Card className={`h-full flex flex-col overflow-hidden transition-all duration-500 glass-card ${isWinner ? winnerBorder : 'opacity-80'}`}>
                ...
                <CardContent className="pt-6 pb-6 space-y-7 flex-1 flex flex-col justify-between">
                    ...
                    <div className="space-y-3">
                        <div className="flex items-end justify-between min-h-[44px]">
                            <div className="space-y-1">
                                ...
                                <div className="flex items-baseline gap-1">
                                    <span className={`text-xl font-bold font-mono tabular-nums ${wins.ctr === type ? colorClass : 'text-foreground/90'}`}>
                                        {data.ctr > 0 ? data.ctr.toFixed(2) : '-'}
                                    </span>
                                    {data.ctr > 0 && (
                            ...
                            <div className="h-6 flex items-end justify-end">
                                {wins.ctr === type && (
                                    <span className={`text-[9px] font-bold uppercase tracking-tighter whitespace-nowrap ${colorClass}`}>
                                        Maior engajamento
                                    </span>
                                )}
                            </div>
                        ...
                        <Progress
                            value={calcProgress(data.ctr, otherData.ctr)}
                            className="h-1.5 bg-white/5"
                            indicatorClassName={wins.ctr === type ? `bg-gradient-to-r ${isVideo ? 'from-violet-600 to-violet-400' : 'from-blue-600 to-blue-400'}` : 'bg-muted-foreground/20'}
                        />
                    </div>
    ...
    const videoTotalWins = (wins.cpl === 'video' ? 1 : 0) + (wins.conversion === 'video' ? 1 : 0) + (wins.ctr === 'video' ? 1 : 0);
    const imageTotalWins = (wins.cpl === 'image' ? 1 : 0) + (wins.conversion === 'image' ? 1 : 0) + (wins.ctr === 'image' ? 1 : 0);
