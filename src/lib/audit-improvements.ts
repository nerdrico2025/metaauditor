...
export function buildCopyChecks(creative: CreativeFields | null | undefined): CopyCheck[] {
...
export function buildImprovementActions(input: {
    ...
    const copyChecks = buildCopyChecks(creative);
    ...
    if (isBranding) {
        for (const rule of brandingResults.filter(r => !r.passed)) {
            addAction(actions, seen, {
                id: `brand-${rule.rule_name}`,
                priority: 'alta',
