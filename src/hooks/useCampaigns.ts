...
export function useCampaigns(filters: CampaignFilters = {}) {
    ...
    return useQuery({
        ...
        queryFn: async (): Promise<{ campaigns: Campaign[]; total: number }> => {
            ...
            // Only show campaigns from monitored integrations (client-side filter to avoid RLS issues)
            const { data: allIntegrations } = await supabase
                .from('integrations')
                .select('id, is_monitored')
                .eq('company_id', companyId);
            const monitoredIds = (allIntegrations || []).filter((i: any) => i.is_monitored === true).map((i: any) => i.id);
            if (monitoredIds.length === 0) return { campaigns: [], total: 0 };
...
export function useActiveCampaignsCount() {
    ...
    return useQuery({
        ...
        queryFn: async (): Promise<number> => {
            ...
            // Only count campaigns from monitored integrations (client-side filter to avoid RLS issues)
            const { data: allIntegrations } = await supabase
                .from('integrations')
                .select('id, is_monitored')
                .eq('company_id', companyId);
            const monitoredIds = (allIntegrations || []).filter((i: any) => i.is_monitored === true).map((i: any) => i.id);
            if (monitoredIds.length === 0) return 0;
