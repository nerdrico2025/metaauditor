...
function applyCreativeScope<T extends ReturnType<typeof supabase.from>>(
    query: T,
    companyId: string,
    scope: { monitoredIntegrationIds: string[] },
    campaignId?: string,
): T {
    let q = query.eq('company_id', companyId) as T;

    if (campaignId) {
        q = q.eq('campaign_id', campaignId) as T;
    } else {
        q = q.in('campaigns.integration_id', scope.monitoredIntegrationIds) as T;
    }
