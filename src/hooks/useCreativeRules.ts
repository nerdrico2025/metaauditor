...
export function useCreativeRuleCheck(creativeId: string | null) {
    ...
    const runCheck = useMutation({
        ...
        onSuccess: (data) => {
            ...
            if (data?.cached && evaluated === 0) {
                toast.success(`Verificação reutilizada — ${cached} regra(s) em cache`);
            } else if (cached > 0 && evaluated > 0) {
                toast.success(`${cached} regra(s) reutilizadas, ${evaluated} analisada(s) agora`);
            } else if (evaluated > 0) {
                toast.success('Verificação de regras concluída');
            }
        },
        onError: (error: Error) => {
    ...
    return useQuery({
        ...
        queryFn: async (): Promise<ComplianceSummaryData> => {
            ...
            if (nonCompliant.length > 0) {
                ...
                const nameMap = new Map(((creatives as any[]) || []).map((c: any) => [c.id as string, c.name as string]));
                for (const nc of nonCompliant) {
                    nc.creative_name = (nameMap.get(nc.creative_id) as string) || 'Criativo sem nome';
                }
            }
            ...
            for (const row of (data || [])) {
                if (!map[row.creative_id]) {
                    ...
                    const failedRules = results
                        .filter((r: CreativeRuleCheckResult) => !r.passed)
                        .map((r: CreativeRuleCheckResult) => ({ rule_name: r.rule_name, severity: r.severity, reason: r.reason }));
                    const effectiveStatus = resolveBrandingCheckStatus(row.overall_status, failedRules);
                    map[row.creative_id] = {
